...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  bool _notificationsEnabled = false; // Inicia apagado
  late UserModel _currentUser;
  late String _originalRole;
  late AnimationController _animationController;
  late Animation<double> _fadeAnimation;

  // Stream para el contador de notificaciones no leídas
  ...
  void initState() {
    ...
    // Inicializa el stream desde el servicio
    _unreadCountStream = NotificacionesService.contarNoLeidasStream();
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          _buildNotificationsIcon(), // Icono de notificaciones con StreamBuilder
        ],
      ),
      drawer: _buildDrawer(),
      body: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [const Color(0xFFEF233C), const Color(0xFFEF233C).withOpacity(0.8), Colors.white],
          ),
        ),
        child: SafeArea(
  ...
  // Widget para el icono de notificaciones con contador en tiempo real
      ...
      builder: (context, snapshot) {
        ...
        return ListView(
          ...
          children: [
            ...
            _buildMenuItemWithBadge(Icons.notifications, 'Notificaciones', unreadCount, () {
              Navigator.pop(context);
            ...
            _buildSettingItem(Icons.notifications, 'Notificaciones', _notificationsEnabled, (value) {
              setState(() => _notificationsEnabled = value);
              if (value) {
                // Si se activa, envía una notificación de prueba
                NotificacionesService.enviarNotificacionAleatoria();
            ...
            Stack(clipBehavior: Clip.none, children: [
              ...
              if (badgeCount > 0)
                Positioned(right: -8, top: -4, child: Container(
                  padding: const EdgeInsets.all(4), decoration: const BoxDecoration(color: Color(0xFFEF233C), shape: BoxShape.circle),
                  constraints: const BoxConstraints(minWidth: 16, minHeight: 16),
                  child: Text(badgeCount > 9 ? '9+' : '$badgeCount', style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                )),
            ]),
            const SizedBox(width: 12),
            Expanded(child: Text(title, style: TextStyle(fontSize: 14, fontWeight: badgeCount > 0 ? FontWeight.w600 : FontWeight.w500, color: Colors.grey.shade700))),
            if (badgeCount > 0)
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                decoration: BoxDecoration(color: const Color(0xFFEF233C).withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
                child: Text('$badgeCount', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFFEF233C))),
              ),
          ]),
        ),
      ),
  ...
  Widget _buildLogoutButton() {
    return Padding(
      ...
      child: TextButton.icon(
        ...
        icon: const Icon(Icons.logout), label: const Text('Cerrar Sesión', style: TextStyle(fontWeight: FontWeight.w500)),
        style: TextButton.styleFrom(foregroundColor: const Color(0xFFEF233C)),
      ),
    );
  }

  // ... (El resto de los métodos de la clase: _canChangeRole, _getRoleColor, etc. se mantienen igual)
  bool _canChangeRole() {
  ...
  IconData _getRoleIcon(String rol) {
    switch (rol.toLowerCase()) {
      case 'admin': case 'administrador': return Icons.admin_panel_settings;
      case 'moderador': return Icons.shield;
  ...
  String _getRoleDescription(String rol) {
    switch (rol.toLowerCase()) {
      case 'admin': case 'administrador': return 'Control total del sistema';
      case 'moderador': return 'Gestión de usuarios y contenido';
      default: return 'Acceso estándar';
    }
  }
      ...
      onTap: () {
        ...
        if (homeState != null) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => ItemsListScreen(category: category, currentUser: homeState._currentUser),
            ),
          );
        }
      },
